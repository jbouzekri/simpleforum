...
class SimpleForumTopic extends eZPersistentObject
{
    ...
    public function canRead()
    {
        ...
        if ( !$this->forumNode()->canRead() || 
        	 ...
        	 !self::checkAccess($this->forumNode()) )
        {
            return false;
        }
    ...
    public static function checkAccess(eZContentObjectTreeNode $forumNode, $functionName = 'read')
    {
    	if (!$forumNode)
    	{
    		return false;
    	...
    	$user = eZUser::currentUser();
    	$accessResult = $user->hasAccessTo( 'topic' , $functionName );
    ...
    public static function showModeratedTopics()
    {
    	...
    	if ( !$ini->variable('SiteAccessSettings', 'ShowModeratedTopics') )
    	{
    		return false;
    	}
    ...
    {
        if ( !self::checkAccess($this->forumNode(), 'remove') )
        {
        	return false;
        }
