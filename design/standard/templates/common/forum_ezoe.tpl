...
{def    $plugin_list = ezini('SimpleForumEditorSettings', 'Plugins', 'ezoe.ini',,true() )
        $skin        = ezini('SimpleForumEditorSettings', 'Skin', 'ezoe.ini',,true() )
        $skin_variant = ''
...
{if ezini_hasvariable( 'SimpleForumEditorSettings', 'SkinVariant', 'ezoe.ini',,true() )}
    {set $skin_variant = ezini('SimpleForumEditorSettings', 'SkinVariant', 'ezoe.ini',,true() )}
{/if}
{if ezini_hasvariable( 'SimpleForumEditorSettings', 'Directionality', 'ezoe.ini',,true() )}
    {set $directionality = ezini('SimpleForumEditorSettings', 'Directionality', 'ezoe.ini',,true() )}
{/if}
{if ezini_hasvariable( 'SimpleForumEditorSettings', 'ToolbarAlign', 'ezoe.ini',,true() )}
    {set $toolbar_alignment = ezini('SimpleForumEditorSettings', 'ToolbarAlign', 'ezoe.ini',,true() )}
{/if}

{def    $cur_locale = fetch( 'content', 'locale' )
        $atr_locale = fetch( 'content', 'locale', hash( 'locale_code', $ez_locale ) )}
...
var eZOeAttributeSettings, eZOeGlobalSettings = {ldelim}
    ...
    theme_ez_statusbar_open_dialog : {cond( ezini('SimpleForumEditorSettings', 'TagPathOpenDialog', 'ezoe.ini',,true())|eq('enabled'), 'true', 'false' )},
...
<div class="oe-window">
    <textarea class="box" id="simpleforum_data_text_{$ez_object.node_id}" name="{$textarea_name}" cols="88" rows="{$editorRow}">{$input_handler.input_xml}</textarea>
</div>
...
<div class="block">
    ...
    eZOeToggleEditor( 'simpleforum_data_text_{$ez_object.node_id}', eZOeAttributeSettings );
    </script>
</div>